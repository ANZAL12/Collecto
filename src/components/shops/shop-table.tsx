...
export function ShopTable({
  ...
  const handleConfirmDeleteFiltered = async () => {
    if (!onDeleteFilteredShops || filteredShops.length === 0) return;
    setIsDeletingBulk(true);
    try {
      await onDeleteFilteredShops(filteredShops);
  ...
  return (
    <>
      <Card className={className}>
        <CardHeader className="py-3 px-4 border-b border-border space-y-3">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 pt-1">
            ...
            {/* Delete All / Delete Filtered Button */}
            {onDeleteFilteredShops && filteredShops.length > 0 && (
              <Button
                type="button"
                variant="outline"
                size="sm"
                onClick={() => setShowDeleteModal(true)}
                className="h-8 text-xs font-mono text-destructive hover:bg-destructive/10 border-destructive/30 hover:border-destructive/60 gap-1.5 shrink-0 ml-auto"
                title={
                  hasActiveFilters
                    ? `Delete ${filteredShops.length} filtered shops`
                    : `Delete all ${shops.length} registered shops`
                }
              >
                <Trash2 className="h-3.5 w-3.5 text-destructive" />
                <span>
                  {hasActiveFilters
                    ? `Delete Filtered (${filteredShops.length})`
                    : `Delete All (${shops.length})`}
                </span>
              </Button>
            )}
          </div>
                      ...
                      {onDeleteShop && (
                        <TableCell className="text-right">
                          <button
                            ...
                            disabled={
                              deletingShopId === `${shop.id}_${shop.mappingId || shop.companyName || ""}` ||
                            ...
                            title={`Delete "${shop.name}"${shop.companyName || shop.brandName ? ` (${shop.companyName || shop.brandName})` : ""}`}
                          >
                            {deletingShopId === `${shop.id}_${shop.mappingId || shop.companyName || ""}` ||
                            deletingShopId === shop.id ? (
      ...
      {/* Confirmation Modal for Delete All / Delete Filtered */}
      {showDeleteModal && (
          ...
          <Card className="w-full max-w-md border-border bg-card p-5 shadow-2xl space-y-4 animate-in zoom-in-95 duration-150">
            <div className="flex items-start gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h3 className="text-sm font-bold text-foreground">
                  {hasActiveFilters
                    ? `Delete Filtered Shops (${filteredShops.length})`
                    : `Delete All Registered Shops (${shops.length})`}
                </h3>
            ...
            <div className="space-y-2 text-xs bg-destructive/5 border border-destructive/20 p-3 rounded-lg text-foreground">
              <p className="font-semibold text-destructive">
                Are you sure you want to delete {filteredShops.length} {filteredShops.length === 1 ? "shop" : "shops"}?
              </p>
              <p className="text-[11px] text-muted-foreground">
                All selected shop master records and their executive mappings will be permanently removed.
              </p>
              ...
              {hasActiveFilters && (
                <div className="mt-2 pt-2 border-t border-destructive/15 space-y-1 text-[11px]">
                  ...
                  <ul className="list-disc pl-4 text-muted-foreground space-y-0.5 font-mono">
                    {search.trim() && <li>Search: &quot;{search.trim()}&quot;</li>}
                    {currentStatusFilter !== "ALL" && <li>Status: {currentStatusFilter}</li>}
                    {executiveFilter !== "ALL" && <li>Executive: {executiveFilter}</li>}
                    {companyFilter !== "ALL" && <li>Company/Brand: {companyFilter}</li>}
                  </ul>
                </div>
              )}

              {/* Sample list of shops being deleted */}
              <div className="mt-2 pt-2 border-t border-destructive/15 text-[11px] text-muted-foreground">
                <span className="font-medium text-foreground">Shops affected: </span>
                <span className="font-mono">
                  {filteredShops.slice(0, 5).map((s) => s.name).join(", ")}
                  {filteredShops.length > 5 ? ` and ${filteredShops.length - 5} more...` : ""}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-2 pt-2 border-t border-border">
              ...
              <Button
                ...
                variant="destructive"
                size="sm"
                onClick={handleConfirmDeleteFiltered}
                disabled={isDeletingBulk}
              ...
              >
                ...
                <span>
                  {isDeletingBulk
                    ? "Deleting..."
                    : `Delete ${filteredShops.length} ${filteredShops.length === 1 ? "Shop" : "Shops"}`}
                </span>
              </Button>
            </div>
          </Card>
